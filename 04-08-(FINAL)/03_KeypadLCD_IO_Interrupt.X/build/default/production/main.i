...
const unsigned int magazineStepperDelay = 1300;
...
void mainMenu(unsigned char *menu){
 ...
 LATCbits.LATC5 = 1;
    while (!PORTBbits.RB1){
        ...
        I2C_Master_Stop();
        { lcdInst(0x80 | LCD_LINE1_ADDR);};
        printf("20%02x-%02x-%02x", time[6],time[5],time[4]);
  ...
  if (PORTAbits.RA1){
            ...
            I2C_Master_Write(0b11010001);
            for(unsigned char i = 0; i < 6; i++){
                startTime[i] = I2C_Master_Read(0);
            }
            startTime[6] = I2C_Master_Read(1);
            I2C_Master_Stop();
...
void topMagStepper(unsigned int steps, unsigned char forward){
    ...
    while ((forward && (topMagCurrentStep < steps)) || (!forward && (topMagCurrentStep > 0))){
        ...
        _delay((unsigned long)((magazineStepperDelay)*(10000000/4000000.0)));
    }
    LATBbits.LATB0 = 0;
    LATBbits.LATB2 = 0;
    LATBbits.LATB4 = 0;
...
void start(unsigned char *menu, run *currentRun, unsigned char *topMagTires, unsigned char *bottomMagTires){
    ...
    _Bool armReached = 1;
 _Bool bottomMagPushed = 1;
 _Bool tireOnBottomRamp = 1;
 ...
 LATCbits.LATC5 = 0;
    ...
    while (1){
        ...
        _delay((unsigned long)((350)*(10000000/4000.0)));
        ...
        if (!tireInCell && !tireOnBottomRamp){
            if (*bottomMagTires > 0){
                bottomMagStepper(singleTireSteps, 1);
                (*bottomMagTires)--;
    bottomMagPushed = 1;
            } else if (*topMagTires > 0){
                topMagStepper(singleTireSteps, 1);
                (*topMagTires)--;
    bottomMagPushed = 0;
            }
  }
  ...
  distTravelled = ((I2C_Master_Read(0) << 8) & 0xff00) | (I2C_Master_Read(1) & 0xff);
  I2C_Master_Stop();
  ...
  if (!tireInCell){
   ...
   _delay((unsigned long)((2050)*(10000000/4000.0)));
   tireInCell = 1;
  }
    ...
    return;
            ...
            if (tiresBefore >= tiresNeeded){
                ...
                if ((tiresNeeded - tiresBefore) == 1){
                    ...
                    _delay((unsigned long)((10)*(10000000/4000.0)));
                    LATAbits.LATA0 = 0;
                    _delay((unsigned long)((1150)*(10000000/4000.0)));
                    currentTires++;
                    tireInCell = 0;
     ...
     if (tireOnBottomRamp){
                        _delay((unsigned long)((1250)*(10000000/4000.0)));
      LATEbits.LATE2 = 1;
      _delay((unsigned long)((10)*(10000000/4000.0)));
      LATEbits.LATE2 = 0;
      tireOnBottomRamp = 0;
...
void runFinished(unsigned char *menu, run *currentRun, unsigned char *polePage, run *displayRun, _Bool *logs){
    int startSec = 0;
    int endSec = 0;
 ...
 writeByteToDriveArduino(6);
    ...
    I2C_Master_Write(0b11010001);
    for(unsigned char i = 0; i < 6; i++){
        endTime[i] = I2C_Master_Read(0);
    }
    endTime[6] = I2C_Master_Read(1);
    I2C_Master_Stop();
...
void runFinishedMenu(unsigned char *menu, _Bool *logs){
    { lcdInst(0x01); _delay((unsigned long)((5)*(10000000/4000.0)));};
    { lcdInst(0x80 | LCD_LINE1_ADDR);};
...
void runInfoMenu(unsigned char *menu, run *displayRun, _Bool logs){
    { lcdInst(0x01); _delay((unsigned long)((5)*(10000000/4000.0)));};
    { lcdInst(0x80 | LCD_LINE1_ADDR);};
...
void poleInfo(unsigned char *menu, run *displayRun, unsigned char *polePage){
    ...
    switch (keypress){
        ...
        case 0b01100:
            if (*polePage == 0){
                *polePage = displayRun->totalPoleNum - 1;
            } else {
                *polePage = *polePage - 1;
            }
            break;
        case 0b01110:
            if (*polePage == displayRun->totalPoleNum - 1){
                *polePage = 0;
            } else{
                *polePage = *polePage + 1;
            }
